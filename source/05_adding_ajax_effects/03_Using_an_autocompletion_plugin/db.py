...
def autocomplete_widget(field,value):
    response.files.append(URL('static','autocomplete/jquery.autocomplete.js'))
    response.files.append(URL('static','autocomplete/jquery.autocomplete.css'))
    ...
    scr = SCRIPT("jQuery('#%s input').autocomplete({source: %s});" % (id, json(items)))
    wrapper.append(inp)
    wrapper.append(scr)
    return wrapper
